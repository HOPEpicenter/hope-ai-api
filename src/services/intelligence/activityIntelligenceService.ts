...
import type { JourneyTimelineCollection } from "../../domain/memberJourney/memberJourney.aggregate";
...
function toJourneyTimelineCollections(
  timelines: readonly { memberId: string; items: readonly object[] }[]
): JourneyTimelineCollection[] {
  return timelines.map((timeline) => ({
    memberId: timeline.memberId,
    items: timeline.items.map((item) => Object.fromEntries(Object.entries(item)))
  }));
}
...
export function buildActivityIntelligence(
  ...
  const memberJourney = new MemberJourneyService().build({
    memberId: "global",
    formationTimeline: toJourneyTimelineCollections(formationTimeline),
    careTimeline: toJourneyTimelineCollections(careTimeline),
    servingTimeline: toJourneyTimelineCollections(servingTimeline),
    communityTimeline: toJourneyTimelineCollections(communityTimeline),
    givingTimeline: toJourneyTimelineCollections(givingTimeline),
    attendanceTimeline: toJourneyTimelineCollections(attendanceTimeline),
    engagementTimeline: toJourneyTimelineCollections(engagementTimeline),
