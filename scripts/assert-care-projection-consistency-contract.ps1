...
function Post-FormationEvent(
  ...
  Json-Post "$ApiBase/formation/events" @{
    v = 1
    eventId = New-EventId "evt-care-projection-consistency"
    visitorId = $VisitorId
    type = $Type
    occurredAt = New-IsoUtc $OccurredAt
    source = @{
      system = "assert-care-projection-consistency"
      actorId = "ops-user-1"
    }
    data = $Data
  } | Out-Null
...
function Create-CareCandidate {
  $visitor = Json-Post "$ApiBase/visitors" @{
    name = "Care Projection Consistency $([guid]::NewGuid().ToString('N').Substring(0,8))"
    email = "care-projection-consistency+$([guid]::NewGuid().ToString('N'))@example.com"
    source = "assert-care-projection-consistency-contract.ps1"
  }
...
Write-Host "Running care projection consistency regression..."
...
$list = Json-Get "$ApiBase/care/candidates?assignmentState=assigned&assignmentBucket=owned&limit=50"
Assert ($list.ok -eq $true) "candidate list response should be ok"
Assert ($null -ne $list.items) "candidate list items should exist"
...
$unassignedList = Json-Get "$ApiBase/care/candidates?assignmentState=unassigned&assignmentBucket=queue&limit=50"
Assert ($unassignedList.ok -eq $true) "unassigned candidate list response should be ok"
...
Write-Host "OK: care projection consistency regression passed."
