...
function validateBody(body: unknown): { ok: true; value: ResolutionRequest } | { ok: false; error: string } {
  ...
  return { ok: true, value: { eventId, resolution } };
}
...
export type ActivityIntelligenceActorDependencies = {
  resolveAdministrativeOverride?: typeof resolveSixWeekAdministrativeOverride;
  requireStaffActor?: typeof requireMinistryCommunicationStaffActor;
};

export async function resolveActivityIntelligenceActor(
  req: any,
  dependencies: ActivityIntelligenceActorDependencies = {}
): Promise<
  | { ok: true; value: ResolutionActor }
  | { ok: false; status: number; body: Record<string, unknown> }
> {
  const override = await (
    dependencies.resolveAdministrativeOverride ?? resolveSixWeekAdministrativeOverride
  )(req);
  ...
  if (override.actorId) {
    return {
      ...
      value: {
        ...
        administrativeOverrideVerified: override.administrativeOverrideVerified
      }
    };
  }

  const actor = await (
    dependencies.requireStaffActor ?? requireMinistryCommunicationStaffActor
  )(req);
  return actor.ok
    ? { ok: true, value: { actorId: actor.actorId } }
    : actor;
}
...
export async function postActivityIntelligenceResolve(
  ...
  try {
    ...
    const resolvedActor = await resolveActivityIntelligenceActor(req);
    if (!resolvedActor.ok) {
      respond(context, resolvedActor.status, {
        ...resolvedActor.body,
        requestId
      });
      return;
    }
    const actor = resolvedActor.value;
    ...
    if (existingEvent) {
      if (!sameRecord(existingEvent, { visitorId, command, actorId: actor.actorId })) {
      ...
      const sixWeekSync = await applyActivityIntelligenceResolution({
        ...
        administrativeOverrideVerified: actor.administrativeOverrideVerified
      });
