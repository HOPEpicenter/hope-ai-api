...
try {
  ...
  $failures.Add("empty visitor create failed: $(# scripts/smoke-visitor-engagements-e2e.ps1
[CmdletBinding()]
param(
  [string]$BaseUrl = $(if ($env:HOPE_BASE_URL) { $env:HOPE_BASE_URL } else { "http://localhost:3000" }),
  [int]$TimelineLimit = 10
...
if ([string]::IsNullOrWhiteSpace($emptyVisitorId) -or $emptyVisitorId.Length -lt 8) {
  ...
  try {
    ...
    $failures.Add("empty score read failed: $(# scripts/smoke-visitor-engagements-e2e.ps1
[CmdletBinding()]
param(
  [string]$BaseUrl = $(if ($env:HOPE_BASE_URL) { $env:HOPE_BASE_URL } else { "http://localhost:3000" }),
  [int]$TimelineLimit = 10
...
.Exception.Message)") | Out-Null
  ...
  try { if (-not $emptyScore -or ($emptyScore.PSObject.Properties.Name -notcontains "ok") -or ($emptyScore.ok -ne $true)) { $failures.Add("empty score expected ok=true") | Out-Null } } catch { }
  try { if ($emptyScore -and ($emptyScore.PSObject.Properties.Name -contains "v") -and ([int]$emptyScore.v -ne 1)) { $failures.Add("empty score expected v=1") | Out-Null } } catch { }
  try { if ($emptyScore -and ($emptyScore.PSObject.Properties.Name -contains "visitorId") -and ([string]$emptyScore.visitorId -ne $emptyVisitorId)) { $failures.Add("empty score visitorId mismatch") | Out-Null } } catch { }
  ...
  try { if ($emptyScore -and ($emptyScore.PSObject.Properties.Name -contains "lastEngagedAt") -and ($null -ne $emptyScore.lastEngagedAt)) { $failures.Add("empty score expected lastEngagedAt=null") | Out-Null } } catch { }
}
