...
$careRegressionScripts = @(
  ...
  "assert-care-summary-export-list-consistency-contract.ps1",
  "assert-care-projection-integrity-guardrails.ps1"
)
...
pwsh -NoProfile -ExecutionPolicy Bypass `
  -File .\scripts\assert-integration-legacy.ps1 `
  -ApiBaseUrl $BaseUrl

if ($LASTEXITCODE -ne 0) {
  throw "assert-integration-legacy.ps1 failed"
}

Write-Host "[OK] Legacy export function payload regression"
