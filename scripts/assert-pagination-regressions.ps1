...
Invoke-Step "task generation readiness" {
  pwsh -NoProfile -ExecutionPolicy Bypass -File "$PSScriptRoot/assert-task-generation-readiness.ps1" `
    -ApiBase $api `
    -ApiKey $ApiKey
}
...
Invoke-Step "ops task preview simulation" {
  pwsh -NoProfile -ExecutionPolicy Bypass -File "$PSScriptRoot/assert-ops-task-preview-simulation.ps1" `
    -ApiBase $api `
    -ApiKey $ApiKey
}
