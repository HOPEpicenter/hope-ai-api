  ...
  try {
    ...
    context.res = {
      ...
      headers: {
        ...
        "X-HOPE-Product-Use": "Read-only orchestration simulation only. Does not create or persist tasks."
      },
      body: {
        ok: true,
        v: 1,
        schemaVersion: TASK_PREVIEW_SCHEMA_VERSION,
        mode: "read-only",
        ...
        groupedPreviews: groupTaskPreviews(previews),
    ...
    context.res = {
      ...
      body: { ok: false, error: err?.message ?? "ops_task_preview_simulation_error" }
    };
  }
}
