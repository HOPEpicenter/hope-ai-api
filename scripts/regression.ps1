...
Write-Host "[regression] Activity intelligence contract..."
pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "assert-activity-intelligence-contract.ps1") -BaseUrl $BaseUrl -ApiKey $env:HOPE_API_KEY
if ($LASTEXITCODE -ne 0) { throw "Activity intelligence contract failed ($LASTEXITCODE)" }

Write-Host "[regression] Dashboard card contract..."
pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "assert-dashboard-card-contract.ps1") -BaseUrl $BaseUrl -ApiKey $env:HOPE_API_KEY
if ($LASTEXITCODE -ne 0) { throw "Dashboard card contract failed ($LASTEXITCODE)" }
...
# Care assignment endpoint contracts
Write-Host "[regression] Care assignment endpoint contracts..."

pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "assert-care-candidate-assign-endpoint-contract.ps1")
if ($LASTEXITCODE -ne 0) { throw "Care candidate assign endpoint contract failed ($LASTEXITCODE)" }

pwsh -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "assert-care-candidate-unassign-endpoint-contract.ps1")
