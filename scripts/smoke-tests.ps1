...
pwsh -NoProfile -ExecutionPolicy Bypass -File "$PSScriptRoot/assert-formation-profile-rebuild.ps1" -BaseUrl $BaseUrl -ApiKey $ApiKey
if ($LASTEXITCODE -ne 0) { throw "Formation profile rebuild failed (exit=$LASTEXITCODE)" }
...
pwsh -NoProfile -ExecutionPolicy Bypass -File "$PSScriptRoot/assert-formation-profile-audit-list.ps1" -BaseUrl $BaseUrl -ApiKey $ApiKey
if ($LASTEXITCODE -ne 0) { throw "Formation profile audit list failed (exit=$LASTEXITCODE)" }
