...
if (-not $etl.ok) {
  Write-Host ""
  Write-Host "GET /engagements/timeline failed (raw):" -ForegroundColor Yellow
  $etl | ConvertTo-Json -Depth 10 | ForEach-Object { Write-Host param(
  [string]$RepoRoot = (Get-Location).Path,
  [string]$BaseUrl = "http://127.0.0.1:3000/api"
)
...
Ok "Regression checks complete."
  ...
  Fail "GET /engagements/timeline failed."
}
$etlBody = $etl.body
