...
function Create-TestVisitor([string]$Label) {
  $visitor = Json-Post "$ApiBase/visitors" @{
    ...
    source = "assert-care-bulk-assignment-ownership-contract.ps1"
  }
...
Write-Host "Running care bulk assignment ownership regression contract..."
...
$assignedDetailA = Json-Get "$ApiBase/care/candidates/$visitorA"
$assignedDetailB = Json-Get "$ApiBase/care/candidates/$visitorB"

Assert ($assignedDetailA.item.assignedTo -eq "ops-user-2") "visitor A assigned detail should reflect assignedTo"
Assert ($assignedDetailA.item.assignmentState -eq "assigned") "visitor A assigned detail assignmentState should be assigned"
Assert ($assignedDetailA.item.assignmentBucket -eq "owned") "visitor A assigned detail assignmentBucket should be owned"

Assert ($assignedDetailB.item.assignedTo -eq "ops-user-2") "visitor B assigned detail should reflect assignedTo"
Assert ($assignedDetailB.item.assignmentState -eq "assigned") "visitor B assigned detail assignmentState should be assigned"
Assert ($assignedDetailB.item.assignmentBucket -eq "owned") "visitor B assigned detail assignmentBucket should be owned"
...
Write-Host "OK: care bulk assignment ownership regression contract passed."
