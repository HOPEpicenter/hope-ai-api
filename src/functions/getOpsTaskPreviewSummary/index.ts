...
export default async function (context: any, req: any): Promise<void> {
  try {
    ...
    const engagementService = new EngagementsService(
      new EngagementEventsRepository()
    );

    const queue = await buildOpsFollowupsQueue({
      eventsTable,
      profilesTable,
      engagementService,
      limit,
      ...
      includeSynthetic:
        String(readQuery(req, "includeSynthetic") ?? "").trim().toLowerCase() === "true",
    ...
    const plans = previews.map(buildTaskPreviewPlan);
    ...
    context.res = {
      ...
      headers: {
        ...
        "X-HOPE-Product-Use": "Read-only task preview summary only. Does not create or persist tasks."
      },
      body: {
        ok: true,
        v: 1,
        ...
        previewSummary: summarizeTaskPreviews(previews),
        planSummary: summarizeTaskPreviewPlans(plans),
        previews: previews.map(serializeTaskPreview),
        plans
      }
    };
  } catch (err: any) {
    context.res = {
      ...
      body: {
        ok: false,
        error: err?.message ?? "ops_task_preview_summary_error"
      }
    };
  }
}
